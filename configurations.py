...
class EnvSettings:
    ...
    def __init__(self, n_clients, n_rounds, n_epochs, keep_best=True, device='gpu', showplot=False, max_T=830):
        self.mode = None
        ...
        print(f"EnvSetting Device: {self.device}")
...
def init_config(dataset,n_clients,n_rounds,n_epochs):
    #5,5
    #15,5
    #15,5
    if dataset == 'SBM': # Generate graphs
        env_cfg = EnvSettings(n_clients, n_rounds=n_rounds, n_epochs=n_epochs, keep_best=True, device='gpu',  max_T=5600)
...
def init_GNN_clients(last_ne, client_nodes):
    ...
    for i, node in enumerate(client_nodes): 
        ...
        print(f"Assigning actor {client_id} to node {ip} with resource label '{label}'")
        clients.append(
            EdgeDevice.options(resources={label: 1}).remote( #assigning to a unique worker node
                id=client_id, prev_ne=last_ne, subnodes=None
            )
        )
