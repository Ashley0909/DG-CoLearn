...
def load_lp_data(task_cfg):
    ...
    train_list, val_list, test_list, data_size = partition_lp_data("ideal", num_snapshots, data)
...
def load_nc_data(task_cfg):
    ...
    """ Split each snapshot into train, val and test """
    train_list, val_list, test_list, data_size = partition_nc_data(num_timesteps, graph_snapshots)
...
def partition_nc_data(num_snapshots, data):
    train_list, val_list, test_list, data_size = [], [], [], []
    for i in range(num_snapshots - 2):
        g_t = copy.deepcopy(data[i])
        g_tplus1 = copy.deepcopy(data[i+1])
        g_tplus2 = copy.deepcopy(data[i+2])

        train_list.append(g_t)
        val_list.append(g_tplus1)
        test_list.append(g_tplus2)
        data_size.append(len(g_t.num_nodes) + len(g_tplus1.num_nodes) + len(g_tplus2.num_nodes)) # Set the data size as the size of nodes
...
def formal_data_splitting(env_cfg, data, mode):
    """ Partition data in train, val and test according to different scenarios:
        'real-life':      clients do not have t+1 data to validation (train and val both in time t)
        'test-temporal':  clients test the models on generalising future performance (val and test both in time t+1)
        'ideal':          clients train using t, val using t+1 and test using t+2

        Distribute nodes in each snapshot
        ('E', None):        Equal Distribution
        ('N', sig):         Normal Distribution (mu=num_nodes/N, sigma=sig*mu)
        ('Label', y):       Label-Skew Distribution, (y is the node label list)
        ('Min_Cut', ratio): Graph Partitioning according to Minimum Cut Problem
    """

    all_modes = {"real-life", "test-temporal", "ideal"}
    all_split_types = {"E", "N", "Label", "Min_Cut"} # Equal, Normal, Label-Skew or Min_Cut
    train_list, val_list, test_list, num_previous_edges = [], [], [], [0]

    g = copy.deepcopy(data[0]) # since node will not change over time, we can just take the first (any) snapshot
    data_size = g.num_nodes

    if mode == "real-life":
        # Split graph into 2 parts: 75% train and 25% val (Global Node Index)
        train_nodes, val_nodes = partition_data(2, data_size, (env_cfg.data_dist[0], 0.75)) 
        test_nodes = torch.arange(data_size)

        # Split train and val data to clients (test data, we take all train and val indices)
        client_train_idx = partition_data(env_cfg.n_clients, train_nodes, (env_cfg.client_dist[0], 0.5), overlap=True)
        client_val_idx = partition_data(env_cfg.n_clients, val_nodes, (env_cfg.client_dist[0], 0.5), overlap=True)
    elif mode == "test-temporal":
        # Split graph into 2 parts: 50% val and 50% test (Global Node Index)
        train_nodes = torch.arange(data_size)
        val_nodes, test_nodes = partition_data(2, data_size, (env_cfg.data_dist[0], 0.5))
    elif mode == "ideal": # Global Node Index
        train_nodes, val_nodes, test_nodes = torch.arange(data_size), torch.arange(data_size), torch.arange(data_size)

    
    for i in range(env_cfg.n_clients):
        client_train = train_nodes[client_train_idx[i]]
        client_val = val_nodes[client_val_idx[i]]

        # Set number of total subnodes per client (Need for PE dimension)      

def partition_data(n, data_size, split_type, overlap=False):
    """ Split the data into n parts (in indices) according to the split_type.
        n:          can represent number of clients, or (train/val) = 2 or (val/test) = 2 or (train/val/test) = 3
        data_size:  number of nodes in the global graph
        split_type: a tuple that represent (type, ratio of split), or (type, sig) if type == 'N'
        overlap:    decides whether the client subgraphs can be overlapped (definite False for train/val/test split)
    """
    rand_node_idc = torch.randperm(data_size).tolist()
    subnodes_list, subnodes_split = [], [0]

    if split_type[0] == 'E':
        subnode_eq = int(data_size / n)
        for i in range(n):
            subnodes_split.append((i+1) * subnode_eq)
        ...
        mu = data_size / n
        sigma = split_type[1] * mu
        shard_sizes = generate_shard_sizes(n, data_size, mu, sigma)

        last_point = 0
        for split in shard_sizes:
            subnodes_split.append(last_point + int(split))
            last_point += int(split)
        subnodes_split[-1] = data_size
    ...
    elif split_type[0] == 'Label':
        y = split_type[1]
        unique_labels = torch.unique(y)
        node_by_label = defaultdict(list)
        node_indices = np.arange(data_size)
        for node in node_indices:
            node_by_label[y[node].item()].append(node)
        ...
        for i in range(n):
            ...
            pick_nodes = list(set(pick_nodes) - all_assigned_nodes)
            majority_sample = math.floor(len(pick_nodes) * 0.8) # Party takes 80% of the majority data to form its dataset
            other_samples = math.ceil(len(pick_nodes) * 0.2)    # For the remaining data, we take from other labels
            subnodes = random.sample(pick_nodes, majority_sample)
            all_assigned_nodes.update(subnodes)
            other_nodes = random.sample(np.setdiff1d(node_indices, list(all_assigned_nodes)).tolist(), other_samples)
            all_assigned_nodes.update(other_nodes)
            subnodes = subnodes + other_nodes
            subnodes_list.append(subnodes)
            print(f'Party {i} has {len(subnodes)} nodes')
            
            # # Set previous embedding for each client (In NC, prev embedding shape varies among clients)
            # hidden_conv1, hidden_conv2 = 64, 32
            # clients[i].prev_ne = [torch.Tensor([[0 for _ in range(hidden_conv1)] for _ in range(len(subnodes))]), torch.Tensor([[0 for _ in range(hidden_conv2)] for _ in range(len(subnodes))])] # Set the PE shape of each client to be [#subnodes, in_dim] respectively

    else:
        """ Write the graph partitioning method for Min_Cut """
