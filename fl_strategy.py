...
def run_dygl(env_cfg, task_cfg, server, global_mod, cm_map, fed_data_train, fed_data_val, fed_data_test, snapshot, client_shard_sizes, data_size, test_ap_fig, test_ap, tot_num_nodes, log_file, max_i):
   # Initialise
   ...
   t1 = time.perf_counter()
   log_file.write(f"[SNAPSHOT {snapshot}/{max_i}] Time taken for Distributing Models: {t1 - t0}\n")
   ...
   t1 = time.perf_counter()
   log_file.write(f"[SNAPSHOT {snapshot}/{max_i}] Time taken for Sampling Clients: {t1 - t0}\n")
   ...
   log_file.write(f"[SNAPSHOT {snapshot}/{max_i}] Time taken for Initializing Optimizers: {end_time_optimizers - start_time_optimizers}\n")

   start_time_schedulers = time.perf_counter()
   schedulers = {}
   for i in client_ids:
   ...
   end_time_schedulers = time.perf_counter()
   log_file.write(f"[SNAPSHOT {snapshot}/{max_i}] Time taken for Initializing Schedulers: {end_time_schedulers - start_time_schedulers}\n")
   ...
   print(f"Time taken for Global Node State Computation: {end_time_global - start_time_global}")
   for c in fed_data_train:
   ...
   log_file.write(f"[SNAPSHOT {snapshot}/{max_i}] Time taken for Pretraining Communication: {end_time_pretrain - start_time_pretrain}\n")
   ...
   for rd in range(env_cfg.n_rounds):
      ...
      for epoch in range(env_cfg.n_epochs):
         ...
         torch.cuda.synchronize()
         t1 = time.perf_counter()
         log_file.write(f"[SNAPSHOT {snapshot}/{max_i}, Round {rd}/{env_cfg.n_rounds}, Epoch {epoch}/{env_cfg.n_epochs}] Train time: {t1 - t0}\n")
         ...
         torch.cuda.synchronize()
         t1 = time.perf_counter()
         log_file.write(f"[SNAPSHOT {snapshot}/{max_i}, Round {rd}/{env_cfg.n_rounds}, Epoch {epoch}/{env_cfg.n_epochs}] Local test and update metrics time: {t1 - t0}\n")
      ...
      torch.cuda.synchronize()
      end_time_agg = time.perf_counter()
      log_file.write(f"[SNAPSHOT {snapshot}/{max_i}, Round {rd}/{env_cfg.n_rounds}] Aggregation time: {end_time_agg - start_time_agg}\n")
