...
def ml_flow_LP_objective(trial):
    ...
    task_cfg.lr = trial.suggest_loguniform("learning_rate", 1e-5, 1e-1)

    overall_acc, overall_metric = 0, 0    
    ...
    for i in range(num_snapshots-2): # only (num_snapshots - 2) training rounds because of TVT split
        ...
        global_snapshot_acc, global_snapshot_metric, glob_model, best_round, best_metric, val_fig, test_ap_fig, test_ap = run_dygl(env_cfg, task_cfg, server, glob_model, clients, cindexmap, fed_data_train, fed_data_val, fed_data_test, 
                                                                    i, client_shard_sizes, data_size, test_ap_fig, test_ap)
        overall_acc += global_snapshot_acc
        overall_metric += global_snapshot_metric
        print("Snapshot Ends. Best Round:", best_round, "Best Metric (Macro F1):", best_metric)
        print("=============")
        ...
        with mlflow.start_run():
            ...
            mlflow.log_metric(f"Global Best F1 for Snapshot {i}", global_snapshot_metric)
    ...
    with mlflow.start_run():
        ...
        mlflow.log_metric(f"Global Best F1 overall averaged with lr {task_cfg.lr}{i}", overall_metric/(num_snapshots-2))

    return 0.5 * overall_acc/(num_snapshots-2) + 0.5 * overall_metric/(num_snapshots-2)
    
def main():
    # ml flow hyperparameter case study
    mlflow.set_experiment('DBLP5')
